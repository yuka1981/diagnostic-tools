...
RSpec.describe "Tasks", type: :system do
  ...
  describe "accordion row expansion", :js do
    ...
    it "expands row details when clicking" do
      ...
      expect(page).not_to have_css("#task_benchmark_#{benchmark_run.id}_details :not(.hidden)")
      ...
      expect(page).to have_css("#task_benchmark_#{benchmark_run.id}_details :not(.hidden)", wait: 5)
    end
    ...
    it "collapses row details when clicking again" do
      ...
      find("#task_benchmark_#{benchmark_run.id}").click
      sleep 0.5 # Allow animation to complete
      expect(page).not_to have_css("#task_benchmark_#{benchmark_run.id}_details :not(.hidden)", wait: 5)
    end
  ...
  describe "accordion expansion after Turbo Frame refresh", :js do
    ...
    describe "basic accordion functionality" do
      it "expands row when clicking and shows chevron rotation" do
        ...
        # Details row should be collapsed initially (grid wrapper has 0fr)
        expect(page).not_to have_css("#task_benchmark_#{benchmark_run1.id}_details :not(.hidden)")
        ...
        expect(page).to have_css("#task_benchmark_#{benchmark_run1.id}_details :not(.hidden)", wait: 5)
      ...
      it "collapses row when clicking the same row again" do
        ...
        expect(page).not_to have_css("#task_benchmark_#{benchmark_run1.id}_details :not(.hidden)", wait: 5)
      ...
      it "closes previously opened row when clicking a different row (exclusive mode)" do
        ...
        expect(page).to have_css("#task_benchmark_#{benchmark_run2.id}_details :not(.hidden)")
    ...
    describe "accordion after pagination (Turbo Frame refresh)" do
      ...
      it "accordion expansion works after navigating to next page" do
        ...
        expect(page).not_to have_css("##{details_id} :not(.hidden)")
        ...
        expect(page).to have_css("##{details_id} :not(.hidden)", wait: 5)
      ...
      it "exclusive mode works after pagination" do
        ...
        first_row.click
        sleep 0.5 # Allow animation to complete
        expect(page).to have_css("##{first_row_id}_details :not(.hidden)", wait: 5)
        ...
        # First row should be collapsed
        expect(page).not_to have_css("##{first_row_id}_details :not(.hidden)", wait: 5)

        # Second row should be expanded
        expect(page).to have_css("##{second_row_id}_details :not(.hidden)")
      end
    end
