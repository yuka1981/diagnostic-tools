...
RSpec.describe Seeds::ServerProductsSeeder do
  describe "#call" do
    ...
    context "with image attachment" do
      ...
      before do
        File.write(json_path, json_data.to_json)
        File.binwrite(test_image_path, minimal_png_data)
      end
      ...
      it "pre-generates image variants" do
        # Use a real PNG file that can be processed by vips
        real_png_path = Rails.root.join("spec/fixtures/files/test_server.png")
        FileUtils.cp(real_png_path, test_image_path)

        seeder.call
        product = ServerProduct.find_by(name: "QuantaGrid D54Q-2U")
        blob = product.images.first.blob
        # Check that the thumbnail variant was pre-generated by verifying a variant record exists
        expect(blob.variant_records.count).to eq(1)
      end
    end
